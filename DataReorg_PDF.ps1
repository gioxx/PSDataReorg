<#
	PSDataReorg: Moving PDF files from "root" to folders and subfolders (Year/Month)
	----------------------------------------------------------------------------------------------------------------
	Author:				GSolone
	Version:			0.10
	Usage:				.\DataReorg_PDF.ps1 $SourceFolder $DestFolder $Year
						(optional, recursive) .\DataReorg_PDF.ps1 $SourceFolder $DestFolder $Year -Recursive
						(example) .\DataReorg_PDF.ps1 C:\temp C:\temp 2020 -Recursive
...
$textFilter = "N.ro e data documento"
$filesFilter = "*.pdf"
$filesCounter = 0
